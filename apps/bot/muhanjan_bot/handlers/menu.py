...
async def _send_main_screen(message: Message, state: FSMContext) -> None:
    ...
    if data.get("is_banned"):
        reason_text = texts.BANNED_REASON_BLOCK.format(
            reason=html_safe(format_ban_reason((await fetch_user_state(message.from_user.id)).get("ban_reason")))
        )
...
async def status_command(message: Message) -> None:
    ...
    if not state.get("exists"):
        await message.answer("Пока не удалось получить твой статус. Попробуй ещё раз.")
        return
